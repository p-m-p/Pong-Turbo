...
const GHOST_COUNT = 4;
const ALIEN_COUNT = 30; // 5 cols × 6 rows
...
function maxScoreForLevel(level) {
  ...
  if (isBonus) {
    // max alien combo: all 30 at once → level × g × 30 × 2^29 (impossible, but we cap generously)
    ...
    // max ghost combo: all 4 at once → level × g × 4 × 2^3 = level × g × 32
...
function validateCheckpoints(checkpoints, finalScore) {
  ...
  for (const cp of checkpoints) {
    if (
      ...
      !Number.isInteger(cp.level) ||
      cp.level < 1 ||
      typeof cp.score !== 'number' ||
      !Number.isInteger(cp.score) ||
      cp.score < 0
    )
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'POST' && url.pathname === '/scores') {
      ...
      // ── Mark token used ───────────────────────────────────────────────────
      await env.DB.prepare('UPDATE tokens SET used = 1 WHERE nonce = ?').bind(payload.nonce).run();

      // ── Insert score ──────────────────────────────────────────────────────
      const run = await env.DB.prepare('INSERT INTO scores (name, score, ts) VALUES (?, ?, ?)')
        .bind(safeName, score, Date.now())
        .run();
