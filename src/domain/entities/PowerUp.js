...
export class PowerUp {
  ...
  isLive(now)  { return now - this.#born >= POWERUP_GRACE_MS; }

  /** True when the 10s lifespan has elapsed. */
  expired(now) { return now - this.#born >= POWERUP_LIFESPAN_MS; }

  move(canvasH, timeScale) {
    this.x += this.#vx * timeScale;
    this.y += this.#vy * timeScale;
    if (this.x <= 0)                    { this.#vx =  Math.abs(this.#vx); }
    if (this.x + this.w >= POWERUP_ROAM_RIGHT) { this.#vx = -Math.abs(this.#vx); }
